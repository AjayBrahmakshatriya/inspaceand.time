<?php $ogimg="https://stuck.intimeand.space/images/gamesc.png"; ?>
...
Anyway, if you too are as bored and like those old text based dungeon crawling games, take a stab at <b>pandemic-crawler</b> (play time < 3 mins).
<br>
<br>
<div id="fb-root"></div>
...
window.onload = function() {
	...
	var outerconsolewrapper = document.getElementById("outerconsolewrapper");
	...
	*/
	...
	function logback(val) {
		var oReq = new XMLHttpRequest(); 
		oReq.open("GET", "/game/log.php?name=" + val); 
		oReq.send();
	}
	...
	function inputreceived(val) {
		logback(player_name + ": " + gamestate + " -> " + val);
		switch(gamestate) {
			case 0: if (val=="") { 
					consoleprompt.innerHTML = ">&nbsp;Login name:&nbsp;";
					return;
				}  
				addtextslow("Welcome, " + val + "\n" + gameintroduction, function(){});  
				player_name = val;
				break;
			case 1: if (val=="1") {
					addtextslow("Good!\n\n" + scenariotext + reprompttext, function(){}); 
				} else if (val == "2") {
					addtextslow("Hmm, well! We can hope for the best.\n\n" + scenariotext + reprompttext, function(){}); 
				} else {
					addtextslow("Well, following instruction is hard. But we can hope for the best.\n\n" + scenariotext + reprompttext, function(){}); 
				}
				break;
			case 2: if (val=="2") {
					addtextslow(winmessage, function(){revealmessage();}); 
				} else if (val=="1") {
					addtextslow(losemessage, function(){revealmessage();}); 
				} else {
					addtextslow(reprompttext, function(){}); gamestate -=1;
				} 
				break;
		}
		gamestate+=1;
	}
	outerconsolewrapper.onclick = function(e) {
		typedtext.focus();
		e.stopPropagation();
	}
